...
def build_sheet1(wb):
    ...
    # ── 행 번호 매핑 (수식 참조에 사용) ──
    # 섹션 행
    ws.merge_cells("A3:D3");  _sec(ws["A3"], "▶ 탱크 기하 조건")
    # Row 4 : 탱크 내경
    _lbl(ws.cell(4, 1), "탱크 내경 (직경) [m]")
    _inp(ws.cell(4, 2), 4.0)
    _lbl(ws.cell(4, 3), "m"); _lbl(ws.cell(4, 4), "원통형 가정")
    # Row 5 : 탱크 길이
    _lbl(ws.cell(5, 1), "탱크 길이 (원통부) [m]")
    _inp(ws.cell(5, 2), 10.0)
    _lbl(ws.cell(5, 3), "m"); _lbl(ws.cell(5, 4), "")
    # Row 6 : 충전율
    _lbl(ws.cell(6, 1), "LNG 충전율 [%]")
    _inp(ws.cell(6, 2), 90)
    _lbl(ws.cell(6, 3), "%"); _lbl(ws.cell(6, 4), "0~100")

    ws.merge_cells("A7:D7");  _sec(ws["A7"], "▶ 운전 조건")
    # Row 8 : 압력
    _lbl(ws.cell(8, 1), "탱크 내부 압력 [bar(a)]")
    _inp(ws.cell(8, 2), 1.13)
    _lbl(ws.cell(8, 3), "bar(a)"); _lbl(ws.cell(8, 4), "절대압")
    # Row 9 : 외부 온도
    _lbl(ws.cell(9, 1), "외부(주위) 온도 [℃]")
    _inp(ws.cell(9, 2), 25)
    _lbl(ws.cell(9, 3), "℃"); _lbl(ws.cell(9, 4), "")

    ws.merge_cells("A10:D10"); _sec(ws["A10"], "▶ 단열재 조건")
    # Row 11 : 단열재 종류
    _lbl(ws.cell(11, 1), "단열재 종류")
    _inp(ws.cell(11, 2), "Perlite(진공)")
    _lbl(ws.cell(11, 3), "-"); _lbl(ws.cell(11, 4), "드롭다운 선택")
    # Row 12 : 두께
    _lbl(ws.cell(12, 1), "단열재 두께 [m]")
    _inp(ws.cell(12, 2), 0.30)
    _lbl(ws.cell(12, 3), "m"); _lbl(ws.cell(12, 4), "")
    # Row 13 : 진공도
    _lbl(ws.cell(13, 1), "진공도 [Pa]")
    _inp(ws.cell(13, 2), 1.0)
    _lbl(ws.cell(13, 3), "Pa"); _lbl(ws.cell(13, 4), "진공단열 시 적용")

    ws.merge_cells("A14:D14"); _sec(ws["A14"], "▶ 배관 조건")
    # Row 15 : 배관 보냉 여부
    _lbl(ws.cell(15, 1), "배관 보냉/진공 여부")
    _inp(ws.cell(15, 2), "진공배관")
    _lbl(ws.cell(15, 3), "-"); _lbl(ws.cell(15, 4), "드롭다운 선택")
    # Row 16 : 배관 추가 열유입
    _lbl(ws.cell(16, 1), "배관 추가 열유입 비율 [%]")
    _inp(ws.cell(16, 2), 10)
    _lbl(ws.cell(16, 3), "%"); _lbl(ws.cell(16, 4), "탱크 열유입 대비")

    ws.merge_cells("A17:D17"); _sec(ws["A17"], "▶ 계측 조건")
    # Row 18 : 안정화 시간
    _lbl(ws.cell(18, 1), "안정화 시간 [h]")
    _inp(ws.cell(18, 2), 12)
    _lbl(ws.cell(18, 3), "h"); _lbl(ws.cell(18, 4), "BOG 안정화 대기 시간")
    # Row 19 : 계측 시간
    _lbl(ws.cell(19, 1), "BOG 계측 시간 [h]")
    _inp(ws.cell(19, 2), 1.0)
    _lbl(ws.cell(19, 3), "h"); _lbl(ws.cell(19, 4), "실측 수집 시간")

    ws.merge_cells("A20:D20"); _sec(ws["A20"], "▶ LNG 조성 (상위 3성분)")
    # Row 21 : 조성 헤더
    for j, t in enumerate(["성분 이름", "몰분율 [mol%]", "단위", "비고"], 1):
        _hdr(ws.cell(21, j), t, bg=CLR_DB_HDR_BG)
    # Row 22 : 성분 1
    _inp(ws.cell(22, 1), "Methane");  _inp(ws.cell(22, 2), 90)
    _lbl(ws.cell(22, 3), "mol%");    _lbl(ws.cell(22, 4), "주성분")
    # Row 23 : 성분 2
    _inp(ws.cell(23, 1), "Ethane");   _inp(ws.cell(23, 2), 7)
    _lbl(ws.cell(23, 3), "mol%");    _lbl(ws.cell(23, 4), "")
    # Row 24 : 성분 3
    _inp(ws.cell(24, 1), "Propane");  _inp(ws.cell(24, 2), 3)
    _lbl(ws.cell(24, 3), "mol%");    _lbl(ws.cell(24, 4), "합계 100%")
    ...
    ws.add_data_validation(dv2); dv2.add(ws["B15"])
    ...
    ws.add_data_validation(dv3); dv3.add("B22:B24")
...
def build_sheet3(wb):
    ...
    items = [
        ...
        (R_RADIUS,  "탱크 반지름 r [m]",
         f"={I}!B4/2", "m", "= 직경 / 2", False),
        (R_A_CYL,   "원통부 외면 표면적 [m²]",
         f"=2*PI()*B{R_RADIUS}*{I}!B5", "m²", "= 2π r L", False),
        (R_A_HEMI,  "양단 반구 표면적 [m²]",
         f"=4*PI()*B{R_RADIUS}^2", "m²", "= 4π r²  (양쪽 반구 합)", False),
        (R_A_TOT,   "총 표면적 A [m²]",
         f"=B{R_A_CYL}+B{R_A_HEMI}", "m²", "원통 + 양단 반구", False),
        (R_V_TANK,  "탱크 내부 체적 V [m³]",
         f"=PI()*B{R_RADIUS}^2*{I}!B5+(4/3)*PI()*B{R_RADIUS}^3",
         "m³", "= π r² L + (4/3)π r³", False),
        (R_V_LNG,   "LNG 체적 V_LNG [m³]",
         f"=B{R_V_TANK}*{I}!B6/100", "m³", "= V_tank × 충전율(%)", False),

        (R_SEC_INS, "▶ 단열 열저항 계산", None, None, None, True),
        (R_K_INS,   "단열재 k값 (VLOOKUP) [W/m·K]",
         f"=VLOOKUP({I}!B11,{D}!A4:B9,2,FALSE)",
         "W/m·K", "2_물성DB A4:B9 참조", False),
        (R_VAC_CF,  "진공보정계수 (고진공 시 keff 감소)",
         f"=IF({I}!B13<10,0.7,IF({I}!B13<100,0.85,1.0))",
         "-", "진공도<10Pa→0.7, <100Pa→0.85, 대기→1.0", False),
        (R_K_EFF,   "유효 열전도율 keff [W/m·K]",
         f"=B{R_K_INS}*B{R_VAC_CF}",
         "W/m·K", "= k × 진공보정계수", False),
        (R_R_TH,    "열저항 R [K/W]",
         f"={I}!B12/(B{R_K_EFF}*B{R_A_TOT})",
         "K/W", "= 두께 / (keff × A)", False),

        (R_SEC_LNG, "▶ LNG 조성 가중평균 물성 계산", None, None, None, True),
        (R_HV1,     "성분1 잠열 [kJ/kg]",
         f"=VLOOKUP({I}!A22,{D}!A13:E18,3,FALSE)",
         "kJ/kg", "2_물성DB A13:E18 참조", False),
        (R_HV2,     "성분2 잠열 [kJ/kg]",
         f"=VLOOKUP({I}!A23,{D}!A13:E18,3,FALSE)",
         "kJ/kg", "", False),
        (R_HV3,     "성분3 잠열 [kJ/kg]",
         f"=VLOOKUP({I}!A24,{D}!A13:E18,3,FALSE)",
         "kJ/kg", "", False),
        (R_HV_AVG,  "평균 잠열 ΔHvap [kJ/kg]",
         f"=(B{R_HV1}*{I}!B22+B{R_HV2}*{I}!B23+B{R_HV3}*{I}!B24)"
         f"/({I}!B22+{I}!B23+{I}!B24)",
         "kJ/kg", "몰분율 가중평균", False),
        (R_RHO1,    "성분1 액밀도 [kg/m³]",
         f"=VLOOKUP({I}!A22,{D}!A13:E18,4,FALSE)",
         "kg/m³", "", False),
        (R_RHO2,    "성분2 액밀도 [kg/m³]",
         f"=VLOOKUP({I}!A23,{D}!A13:E18,4,FALSE)",
         "kg/m³", "", False),
        (R_RHO3,    "성분3 액밀도 [kg/m³]",
         f"=VLOOKUP({I}!A24,{D}!A13:E18,4,FALSE)",
         "kg/m³", "", False),
        (R_RHO_AVG, "평균 밀도 ρ [kg/m³]",
         f"=(B{R_RHO1}*{I}!B22+B{R_RHO2}*{I}!B23+B{R_RHO3}*{I}!B24)"
         f"/({I}!B22+{I}!B23+{I}!B24)",
         "kg/m³", "몰분율 가중평균", False),
        (R_TBP1,    "성분1 비등점 [℃]",
         f"=VLOOKUP({I}!A22,{D}!A13:E18,5,FALSE)",
         "℃", "", False),
        (R_TBP2,    "성분2 비등점 [℃]",
         f"=VLOOKUP({I}!A23,{D}!A13:E18,5,FALSE)",
         "℃", "", False),
        (R_TBP3,    "성분3 비등점 [℃]",
         f"=VLOOKUP({I}!A24,{D}!A13:E18,5,FALSE)",
         "℃", "", False),
        (R_TBP_AVG, "평균 비등점 T_bp [℃]",
         f"=(B{R_TBP1}*{I}!B22+B{R_TBP2}*{I}!B23+B{R_TBP3}*{I}!B24)"
         f"/({I}!B22+{I}!B23+{I}!B24)",
         "℃", "몰분율 가중평균", False),
        ...
        (R_PIPE_CF, "배관 보냉계수 (VLOOKUP)",
         f"=VLOOKUP({I}!B15,{D}!A22:B24,2,FALSE)",
         "-", "2_물성DB A22:B24 참조", False),
        (R_Q_PIPE,  "배관 열침투 Q_pipe [W]",
         f"=B{R_Q_TANK}*({I}!B16/100)*B{R_PIPE_CF}",
...
def build_sheet4(wb, row_bor, row_lng_mass):
    ...
    _calc(ws.cell(7, 2), f"=IF({I}!B18<6,1.15,IF({I}!B18<12,1.05,1.0))")
    _lbl(ws.cell(7, 3), "-")
